...
//__IO指示编译器不要因优化而省略此指令，必须每次都直接读写其值。
...
int main(void)
{
    ...
    virtual_GND(); //设置一些gpio为推挽输出下拉，同时初始化LED
    NVIC_conf();
...
/* Run this function prior to datacollection  在收集数据之前运行此函数 */
void flush_CCD()
{
...
void NVIC_conf(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannel = TIM5_IRQn; //设置定时器5中断
    ...
    NVIC_InitStructure.NVIC_IRQChannel = DMA2_Stream0_IRQn; //ADC的DMA中断
    ...
    NVIC_InitStructure.NVIC_IRQChannel = DMA1_Stream5_IRQn; //串口2的DMA接收中断
